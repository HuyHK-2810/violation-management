...
async function saveRecord() {
    ...
    const newRecord = { date, studentName, violations: violations.join(', '), totalDeduction, remainingPoints };
...
async function loadData() {
    ...
    try {
        const response = await fetch(`/api/violations?date=${date}&studentName=${studentName}`);
...
function calculateWeeklyTotal() {
    ...
    const weeklySummary = studentRecords.reduce((summary, record) => {
        if (!summary[record.studentName]) {
            summary[record.studentName] = { totalDeduction: 0, remainingPoints: initialPoints };
        }
        summary[record.studentName].totalDeduction += record.totalDeduction;
        summary[record.studentName].remainingPoints -= record.totalDeduction;
        return summary;
    }, {});
